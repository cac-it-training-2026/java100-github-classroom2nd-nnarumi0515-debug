...
class Robot {//①クラスの宣言
	...
	//フィールドの値が外から変更されるのでsetterメソッド、フィールドの値を代入
	...
	void makeOmelet(int eggNum, int butterNum) {
		int omeNum1 = eggNum / 2;
		int omeNum2 = butterNum / 5;
		if (omeNum1 > omeNum2) {
			System.out.println(omeNum2 + "人分のオムレツを作成しました。\n");

		} else {
			System.out.println(omeNum1 + "人分のオムレツを作成しました。\n");

		}
	}

	int getWater() {//メソッドgetWater、引数なし設定、フィールドの値を取得する
		return water;
	}
	...
	String makeEggDishes(int flourNum, int sugarNum, int eggNum, int butterNum) {
		...
		String menu = null;
		if ((flour >= 0) && (suger >= 0) && (egg >= -1) && (butter >= -75)) {
			menu = "クッキー";

		} else if ((egg >= 0) && (butter >= 0)) {
			menu = "オムレツ";
...
class ClearRobot {
	void clearTable(int[] ingredients) {
		Arrays.fill(ingredients, 0);//Arrays:配列を扱うクラス、fill；埋めるメソッド、配列ingredientsを０で埋める
	}
