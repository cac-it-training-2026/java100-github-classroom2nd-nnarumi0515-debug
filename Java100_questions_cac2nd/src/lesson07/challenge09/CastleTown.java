...
public class CastleTown {
	...
	public static void main(String[] args) {
		...
		//ここに適切な処理を記述
		Samurai[] samuraiArray = new Samurai[5];//Samurai型の配列作成（要素数５）
		for (int i = 0; i < 5; i++) {//5回繰り返し
			int num = (int) (Math.random() * 10) % 2;//ランダムで整数０か１を返す
			if (num == 0) {//numが0の時
				samuraiArray[i] = new Retainer();//配列にRetainerを格納
			} else {
				samuraiArray[i] = new Ronin();//1の時配列にRonin格納
			}
		}
		...
		//ここに適切な処理を記述
		for (int i = 0; i < 5; i++) {
			System.out.println(samuraiArray[i].getClass());//getClass()で配列に格納された要素がどこのクラスのものか返す
		}
