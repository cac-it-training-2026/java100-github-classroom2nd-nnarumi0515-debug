...
class Omegalian {
	...
	public void setItem(String item) {
		int lastIndex = item.length() - 1;//整数型lastIndexに文字列型itemの長さ‐1代入
		char lastch = item.charAt(lastIndex);//charAt(lastIndex)で文字列型itemの最後の文字取り出して代入
		String changeStr = changeLastChar(lastch);//changeLastChar(lastch)呼び出して文字列changeStrに代入

		if (changeStr != null) {//changeStrがnullでないとき（変換する文字がある時）
			StringBuffer sb = new StringBuffer(item);
			sb.replace(lastIndex, lastIndex + 1, changeStr);//replace(開始位置, 終了位置, 置換文字列)
