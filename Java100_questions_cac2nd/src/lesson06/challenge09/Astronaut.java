...
class Alphalian {
	...
	public Alphalian() {//コンストラクタ生成
		String[] names = { "A", "B", "C", "D", "E" };//配列names用意
		int nameNum = (int) (Math.random() * 10) % 5;//ランダムで0～4の数字指定
		this.name = names[nameNum];//namesの要素代入
	}
...
public class Astronaut {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 10; i++) {
			...
			System.out.println("\nα星人：" + (i + 1) + "人目");
			Alphalian alphalian = new Alphalian();//オブジェクト生成
			String name = alphalian.getName();//getName呼び出し
			if (name.equals(estimatedName)) {//equals()で比較
				hitFlag = true;
				break;
			} else {
